...
abstract class JavascriptRuntime {
  ...
  void _setupConsoleLog() {
    ...
    var console = {
      log: function() {
        callNative('ConsoleLog', JSON.stringify(['log', ...arguments]));
      },
      warn: function() {
        callNative('ConsoleLog', JSON.stringify(['info', ...arguments]));
      },
      error: function() {
        callNative('ConsoleLog', JSON.stringify(['error', ...arguments]));
      }
    }
    var __global = {
      nativeLog: function() {
    ...
    onMessage('ConsoleLog', (dynamic args) {
      WXLog.debug('[JS Calling Flutter]', args);
    });
  }
